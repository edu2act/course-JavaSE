...
public class UserDaoImpl {
	...
	public boolean saveUser(User u){
		try{
			...
			//创建statement
			PreparedStatement pstm=con.prepareStatement(
					"insert into user(nickName,password,introduce,"
					+ "gender,registTime,ip) values(?,?,?,?,?,?)",
					Statement.RETURN_GENERATED_KEYS);
			//设置参数，针对？占位符
			pstm.setString(1, u.getNickName());
			pstm.setString(2, u.getPassword());
			pstm.setString(3, u.getIntroduce());
			pstm.setString(4, u.getGender());
			pstm.setString(5, u.getRegistTime().toLocaleString());
			pstm.setString(6, u.getIp());
			//执行sql语句
			int count=pstm.executeUpdate();
			ResultSet rs=pstm.getGeneratedKeys();
			if(rs.next())
				System.out.println("用户："+rs.getInt(1));
			//关闭连接
			ConnectionUtil.closeCon(rs, pstm, con);
			if(count>0)
				return true;
	...
	public List<User> findFriendByQqNum(){
		Connection con=null;
		PreparedStatement pstm=null;
		ResultSet rs=null;
		try{
